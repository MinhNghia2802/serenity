...
export async function GET() {
  ...
  const { data, error } = await context.supabase.from("question_sets").select("*").order("created_at", { ascending: false });
...
export async function POST(request: Request) {
  ...
  const { data, error } = await context.supabase.from("question_sets").insert({ name: input.name, description: input.description, updated_by: context.user.id }).select().single();
